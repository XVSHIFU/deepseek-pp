...
- **命令执行前置待解决**：当前 Windows 官方 ACL 后端报告 partial；受控临时 fixture 实测未满足越界写拒绝，因此不能接入生产 shell 或标记 T4.5 完成。操作者已授权利用现有 WSL2 Ubuntu 评估：已有 Linux Node 24.18.0 和 Bubblewrap 0.11.1，Windows → WSL 的 `127.0.0.1` TCP 连接已通过。Linux 不能复用当前 Windows Koffi 原生模块；按官方参数直接运行 Bubblewrap 仍能启动 Windows `cmd.exe`（仅执行固定 echo），不能因此声称宿主写入已隔离。下一步需明确授权项目专用 Linux 依赖安装及 Ubuntu 互操作配置调整/重启；授权前不安装、不改系统、不接生产命令。
