...
- 用户指出缺图后补齐五张配图：三张当前 Harness 设置实拍、一张用户提供的扩展配对截图、一张安装目录 SVG 示意图。仅保留相关设置区域，令牌被界面隐藏，没有读取或重新生成令牌；“等待重试”示例状态在图注中如实说明。没有为截图改动模型、配对或权限设置。
- 新完整包 `.release/deepseek-web-harness-20260908-r2.zip` 合并 `a636469` 已验证扩展 / Harness 插件、原配套 vendor 和 `5429672` 双语 README / 图片。共 18 文件、27,330,185 字节，SHA-256 `1f4678621391727201c74448e90aa91e0869b3ed79e3eb72b6b7086194568224`；组件来源、ZIP 内容、内部校验值和中英文各 5 个图片引用逐项通过。无图 r2 包已移入 `.release/.backups/r2-before-images-864f923/` 保留，不改动已发布旧包。
