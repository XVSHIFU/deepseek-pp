...
function node {
    ...
    if ($args[0] -ne (Join-Path (Split-Path $smokeScript -Parent) 'dsh-web-real-smoke.mjs')) { throw 'Wrong runner.' }
...
try {
    ...
    Write-Output 'PASS: no-opt-in has no effects; paired launch supplies all configuration without printing the token.'
}
finally {
