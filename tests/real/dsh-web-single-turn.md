...
**本机当前状态（2026-09-05）**：已有两次真实单轮调用成功；`REAL_WEB_SESSION_EVIDENCE_INVALID` 误报已修复，并用原始记录复验通过。本次只修改本地校验脚本，**不需要重装或重新加载扩展、重新配对，也不需要重复单轮测试**。下方步骤保留给其他环境首次测试使用；下一项是本地工具多回合验证。
...
1. **加载带「本机 Harness」设置的 DeepSeek++，登录 DeepSeek 网页。**
   ...
   已按下方目录安装并启用 `a5b9380` 开发版的，无需再安装或重新加载。只有仍运行更早构建时，才需在扩展管理页点击该扩展的「重新加载」，再刷新 DeepSeek 网页。原仓库版本没有「本机 Harness」，首次测试时关闭原版本，保留其数据。
