...
This fork's `feature/web-harness` branch provides an incremental plugin for the official DeepSeek Harness `web` profile (Mode A). Install the matching browser extension and DSH plugin, pair them once in the settings UI, then run `dsh web` from your project directory to use the official Harness web interface. Other official model providers remain available; making DeepSeek Web the default for future sessions is an explicit choice.

The model is the signed-in DeepSeek website, kept open in the paired browser on the same computer. **No model API key or model configured in Pi is required.** Harness owns sessions, tools, and multi-step execution. Windows commands use PowerShell 7 without WSL; they are disabled by default and require explicit opt-in for new sessions, with per-command approval by default. They run with your Windows-user permissions, not a workspace sandbox. Linux retains official Bash, sandbox, and approval behavior and requires a working Bubblewrap or Landlock backend; it does not use the Windows command setting.

Use Node.js 24.x and Harness `0.1.2-rc.1`. The [main README](README.md) contains the current Windows and Linux source-installation, pairing, and daily-use instructions in Chinese. This is not a public release, and the upstream store extension does not contain this branch's connection feature. Windows + Chrome has passed real pairing, reading, follow-up, editing, PowerShell approval, and cold-restart checks. The current official plugin has **not** completed Linux real-web end-to-end acceptance; historical standalone Linux tests are not evidence of that. Uploading this branch's source does not publish downloadable release packages.
